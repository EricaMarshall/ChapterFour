...
offsetting_Abundance <- function(baseRaster,newRaster, weightRaster, devRaster, 
                                 mainDir = "Z:/Erica/CHAPTER_3",
                                 subDir = "outputDirectory", offPatches=10, bufferZone=15000,
                                 type="value",threshold=0.5,SDM,
  ...
  for(t in x){
    ...
    if(length(of_indices) > 1 || !is.null(of_indices)){
      ...
      theOriginalRaster[of_indices] <- values(baseRaster)[of_indices]
      values(fullRaster2)[unlist(cellFromPolygon(fullRaster2, dev[t,]))] <- NA
      fullRaster2[of_indices] <- values(SDM)[of_indices]
      writeRaster(theOriginalRaster/100, paste0("Abund_patch_", t, ".tif"), overwrite = TRUE)
  ...
  dtf <- data.frame(Run = Repeat,
                    Unoffset = Abundance)
  saveRDS(dtf, file ="Abundance_notoffset.rds")
  fullRaster[oo] <- values(SDM)[oo]
  theOriginalRaster2[oo] <- values(baseRaster)[oo]
  theOriginalRaster2 <- theOriginalRaster2/100
